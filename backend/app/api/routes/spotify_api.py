...
@router.post("/play")
async def play_endpoint(tracks_ids: list[str], app_jwt: str = Cookie(None), session: AsyncSession = Depends(get_async_session)):
    if not app_jwt:
        raise HTTPException(status_code=401, detail="User not logged in")
    service = SpotifyService(session)
    return await service.play_tracks(app_jwt, tracks_ids)
