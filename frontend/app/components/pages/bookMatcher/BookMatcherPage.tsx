...
export default function BookMatcherPage({ bookId }: Props) {
    ...
    const { text, loading, error, goNext, goPrev, currentIndex } = useCachedBookPage(numericBookId, 0);
    ...
    useEffect(() => {
        if (!textMatcher.error && textMatcher.data?.length) {
            const playSpotifyParams: PlayRequestData = {
                tracks_ids: textMatcher.data.map(track => track.spotify_id)
            }
            spotify.execute(playSpotifyParams);
        }
    }, [textMatcher.data, textMatcher.error]);
    ...
    return (
        <StyledMainContainer>
            ...
            <StyledBookContainer>
                ...
                <div>Current page: {currentIndex + 1}</div>
